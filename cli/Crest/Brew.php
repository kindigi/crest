...
class Brew
{
    // This is the array of PHP versions that Crest will attempt to install/configure when requested
    ...
    public function getParsedLinkedPhp(): array
    {
        if (! $this->hasLinkedPhp()) {
            throw new DomainException('Homebrew PHP appears not to be linked. Please run [crest use php@X.Y]');
        }
...
%admin ALL=(root) NOPASSWD:SETENV: BREW'.PHP_EOL);
    ...
    /**
     * Tell Homebrew to forcefully remove all PHP versions that Crest supports.
     */
