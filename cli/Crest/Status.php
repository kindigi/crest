...
class Status
{
    ...
    /**
     * Check the status of the entire Crest ecosystem and return a status boolean
    ...
    /**
     * Define a list of checks to test the health of the Crest ecosystem of tools and configs.
     */
    public function checks(): array
    {
        ...
        return [
            [
                'description' => 'Is Crest fully installed?',
                'check' => function () {
                    return $this->crestInstalled();
                },
                'debug' => 'Run `composer require laravel/crest` and `crest install`.',
            ],
            [
                'description' => 'Is Crest config valid?',
                'check' => function () {
                    try {
                        ...
                        foreach (['tld', 'loopback', 'paths'] as $key) {
                            if (! array_key_exists($key, $config)) {
                                $this->debugInstructions[] = 'Your Crest config is missing the "'.$key.'" key. Re-add this manually, or delete your config file and re-install.';
                ...
                'debug' => 'Run `crest install` to update your configuration.',
            ],
            [
            ...
            [
                ...
                'debug' => 'Uninstall Dnsmasq with Brew and run `crest install`.',
            ],
            [
            ...
            [
                ...
                'debug' => 'Uninstall nginx with Brew and run `crest install`.',
            ],
            [
            ...
            [
                ...
                'debug' => 'Uninstall PHP with Brew and run `crest use php@8.2`',
            ],
            [
                'description' => 'Is crest.sock present?',
                'check' => function () {
                    return $this->files->exists(CREST_HOME_PATH.'/crest.sock');
                },
                'debug' => 'Run `crest install`.',
            ],
        ];
    ...
    public function crestInstalled(): bool
    {
        return is_dir(CREST_HOME_PATH)
            && file_exists($this->config->path())
            && is_dir(CREST_HOME_PATH.'/Drivers')
            && is_dir(CREST_HOME_PATH.'/Sites')
            && is_dir(CREST_HOME_PATH.'/Log')
            && is_dir(CREST_HOME_PATH.'/Certificates');
    }
