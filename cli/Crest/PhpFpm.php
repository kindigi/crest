...
class PhpFpm
{
    ...
    public function install(): void
    {
        ...
        // Remove old crest.sock
        $this->files->unlink(CREST_HOME_PATH.'/crest.sock');
        $this->cli->quietly('sudo rm '.CREST_HOME_PATH.'/crest.sock');

        $this->restart();

        $this->symlinkPrimaryCrestSock($phpVersion);
    }

    /**
     * Forcefully uninstall all of Crest's supported PHP versions and configurations.
     */
    public function uninstall(): void
    {
        $this->brew->uninstallAllPhpVersions();
        rename(BREW_PREFIX.'/etc/php', BREW_PREFIX.'/etc/php-crest-bak'.time());
    ...
    public function createConfigurationFiles(string $phpVersion): void
    {
        ...
        // rename (to disable) old FPM Pool configuration, regardless of whether it's a default config or one customized by an older Crest version
        ...
        $contents = str_replace(
            ['CREST_USER', 'CREST_HOME_PATH', 'crest.sock'],
            [user(), CREST_HOME_PATH, self::fpmSockName($phpVersion)],
            $this->files->getStub('etc-phpfpm-crest.conf')
        );
        ...
        $contents = str_replace(
            ['CREST_USER', 'CREST_HOME_PATH'],
            [user(), CREST_HOME_PATH],
        ...
        $this->files->ensureDirExists(CREST_HOME_PATH.'/Log', user());
        $this->files->touch(CREST_HOME_PATH.'/Log/php-fpm.log', user());
    }
    ...
    public function fpmConfigPath(?string $phpVersion = null): string
    {
        ...
        return BREW_PREFIX."/etc/php/{$versionNormalized}/php-fpm.d/crest-fpm.conf";
    }
    ...
    public function isolatedDirectories(): Collection
    {
        return $this->nginx->configuredSites()->filter(function ($item) {
            return strpos($this->files->get(CREST_HOME_PATH.'/Nginx/'.$item), ISOLATED_PHP_VERSION) !== false;
        })->map(function ($item) {
    ...
    public function useVersion(string $version, bool $force = false): ?string
    {
        ...
        try {
            if ($version === $this->brew->linkedPhp() && ! $force) {
                output(sprintf('<info>Crest is already using version: <comment>%s</comment>.</info> To re-link and re-configure use the --force parameter.'.PHP_EOL,
                    $version));
                exit();
            }
        ...
        info(sprintf('Crest is now using %s.', $newVersion).PHP_EOL);
    ...
    /**
     * Symlink (Capistrano-style) a given Crest.sock file to be the primary crest.sock.
     */
    public function symlinkPrimaryCrestSock(string $phpVersion): void
    {
        $this->files->symlinkAsUser(CREST_HOME_PATH.'/'.$this->fpmSockName($phpVersion), CREST_HOME_PATH.'/crest.sock');
    }
    ...
    public function validateRequestedVersion(string $version): string
    {
        ...
        if (! $this->brew->supportedPhpVersions()->contains($version)) {
            throw new DomainException("Crest doesn't support PHP version: {$version} (try something like 'php@8.1' instead)");
        }
    ...
    public static function fpmSockName(?string $phpVersion = null): string
    {
        ...
        return "crest{$versionInteger}.sock";
    }
    ...
    public function utilizedPhpVersions(): array
    {
        ...
        return $this->nginx->configuredSites()->map(function ($file) use ($fpmSockFiles) {
            $content = $this->files->get(CREST_HOME_PATH.'/Nginx/'.$file);
            ...
            foreach ($fpmSockFiles as $sock) {
                if (strpos($content, $sock) !== false) {
                    ...
                    return $this->normalizePhpVersion(str_replace(['crest', '.sock'], '', $sock));
                }
            }
