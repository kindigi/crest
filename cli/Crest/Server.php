...
class Server
{
    ...
    /**
     * Show the Crest 404 "Not Found" page.
     */
     ...
     */
    public static function showDirectoryListing(string $crestSitePath, string $uri)
    {
        $is_root = ($uri == '/');
        $directory = ($is_root) ? $crestSitePath : $crestSitePath.$uri;
    ...
    /**
     * Return the root level Crest site if given the request URI ($_SERVER['REQUEST_URI'])
     ...
     *
     * For use when accessing Crest sites across a local network.
     */
    public static function crestSiteFromIpAddressUri(string $uri, string $tld): ?string
    {
        ...
        foreach ($this->config['paths'] as $path) {
            ...
            foreach ($dirs as $dir) {
                ...
                if ($domain === strtolower($dir)) {
                    ...
                    $crestSitePath = $path.'/'.$dir;
                }
            }

            if ($crestSitePath) {
                return $crestSitePath;
            }
        }
