...
class Site
{
    ...
    private function getLinkNameByCurrentDir(): ?string
    {
        ...
        if ($count > 1) {
            throw new DomainException("There are {$count} links related to the current directory, please specify the name: crest unlink <name>.");
        }
    ...
    /**
     * Pretty print out all links in Crest.
     */
    ...
    /**
     * Pretty print out all parked links in Crest.
     */
    ...
    /**
     * Get the site URL from a directory if it's a valid Crest site.
     */
    public function getSiteUrl(string $directory): string
    {
        ...
        if (! $this->parked()->merge($this->links())->where('site', $directory)->count() > 0) {
            throw new DomainException("The [{$directory}] site could not be found in Crest's site list.");
        }
    ...
    {
        if (! $this->files->isDir(CREST_HOME_PATH)) {
            return;
        }
    ...
    public function resecureForNewConfiguration(array $old, array $new): void
    {
        ...
        foreach ($secured as $url) {
            ...
            if (! empty($siteConf) && strpos($siteConf, '# crest stub: secure.proxy.crest.conf') === 0) {
                // proxy config
                $this->unsecure($url);
                $this->secure(
                    $newUrl,
    ...
    {
        $shouldComment = $new === CREST_LOOPBACK;

        $lookups = [];
        $lookups[] = '~#?listen .*:80; # crest loopback~';
        $lookups[] = '~#?listen .*:443 ssl http2; # crest loopback~';
        $lookups[] = '~#?listen .*:60; # crest loopback~';
    ...
    {
        $caPemPath = $this->caPath('CrestCASelfSigned.pem');
        $caKeyPath = $this->caPath('CrestCASelfSigned.key');
        ...
        $oName = 'Crest CA Self Signed Organization';
        $cName = 'Crest CA Self Signed CN';
        ...
        $this->cli->runAsUser(sprintf(
            ...
            $caExpireInDays, $oName, $cName, 'rootcertificate@laravel.crest', $caKeyPath, $caPemPath
        ));
        $this->trustCa($caPemPath);
    }
    ...
    public function removeCa(): void
    {
        foreach (['pem', 'key', 'srl'] as $ending) {
            $path = $this->caPath('CrestCASelfSigned.'.$ending);
    ...
    public function createCertificate(string $url, int $caExpireInDays): void
    {
        $caPemPath = $this->caPath('CrestCASelfSigned.pem');
        $caKeyPath = $this->caPath('CrestCASelfSigned.key');
        $caSrlPath = $this->caPath('CrestCASelfSigned.srl');
    ...
    public function createSigningRequest(string $url, string $keyPath, string $csrPath, string $confPath): void
    {
        $this->cli->runAsUser(sprintf(
            ...
            $keyPath, $csrPath, $url, $url, '@laravel.crest', $confPath
        ));
    }
    ...
    {
        info('Trusting Crest Certificate Authority...');
    ...
    {
        $config = str_replace('CREST_DOMAIN', $url, $this->files->getStub('openssl.conf'));
    ...
    public function buildSecureNginxServer(string $url, ?string $siteConf = null): string
    {
        if ($siteConf === null) {
            ...
            $configFile = version_compare($nginxVersion, '1.25.1', '>=') ? 'secure.crest.conf' : 'secure.crest-legacy.conf';
        ...
        return str_replace(
            ['CREST_HOME_PATH', 'CREST_SERVER_PATH', 'CREST_STATIC_PREFIX', 'CREST_SITE', 'CREST_CERT', 'CREST_KEY'],
            [
                $this->crestHomePath(),
                CREST_SERVER_PATH,
                CREST_STATIC_PREFIX,
     ...
     */
    public function isolate(string $crestSite, string $phpVersion): void
    {
        if ($this->files->exists($this->nginxPath($crestSite))) {
            // Modify the existing config if it exists (likely because it's secured)
            $siteConf = $this->files->get($this->nginxPath($crestSite));
            $siteConf = $this->replaceSockFile($siteConf, $phpVersion);
        } else {
            $siteConf = str_replace(
                ['CREST_HOME_PATH', 'CREST_SERVER_PATH', 'CREST_STATIC_PREFIX', 'CREST_SITE', 'CREST_PHP_FPM_SOCKET', 'CREST_ISOLATED_PHP_VERSION'],
                [CREST_HOME_PATH, CREST_SERVER_PATH, CREST_STATIC_PREFIX, $crestSite, PhpFpm::fpmSockName($phpVersion), $phpVersion],
                $this->replaceLoopback($this->files->getStub('site.crest.conf'))
            );
        }

        $this->files->putAsUser($this->nginxPath($crestSite), $siteConf);
    }

    /**
     * Remove PHP Version isolation from a specific site.
     */
    public function removeIsolation(string $crestSite): void
    {
        // If a site has an SSL certificate, we need to keep its custom config file, but we can
        // just re-generate it without defining a custom `crest.sock` file
        if ($this->files->exists($this->certificatesPath($crestSite, 'crt'))) {
            $siteConf = $this->buildSecureNginxServer($crestSite);
            $this->files->putAsUser($this->nginxPath($crestSite), $siteConf);
        } else {
            // When site doesn't have SSL, we can remove the custom nginx config file to remove isolation
            $this->files->unlink($this->nginxPath($crestSite));
        }
    }
    ...
    public function unsecureAll(): void
    {
        ...
        if ($secured->count() === 0) {
            info('No sites to unsecure. You may list all servable sites or links by running <comment>crest parked</comment> or <comment>crest links</comment>.');
    ...
    public function proxyCreate(string $url, string $host, bool $secure = false): void
    {
        ...
        foreach (explode(',', $url) as $proxyUrl) {
            ...
            $nginxVersion = str_replace('nginx version: nginx/', '', exec('nginx -v 2>&1'));
            $configFile = version_compare($nginxVersion, '1.25.1', '>=') ? 'secure.proxy.crest.conf' : 'secure.proxy.crest-legacy.conf';

            $siteConf = $this->replaceOldLoopbackWithNew(
                $this->files->getStub($secure ? $configFile : 'proxy.crest.conf'),
                'CREST_LOOPBACK',
                $this->crestLoopback()
            );

            $siteConf = str_replace(
                ['CREST_HOME_PATH', 'CREST_SERVER_PATH', 'CREST_STATIC_PREFIX', 'CREST_SITE', 'CREST_PROXY_HOST'],
                [$this->crestHomePath(), CREST_SERVER_PATH, CREST_STATIC_PREFIX, $proxyUrl, $host],
                $siteConf
            );
            ...
            info('Crest will now proxy ['.$protocol.'://'.$proxyUrl.'] traffic to ['.$host.'].');
        }
    }
    ...
    public function proxyDelete(string $url): void
    {
        ...
        foreach (explode(',', $url) as $proxyUrl) {
            ...
            info('Crest will no longer proxy [https://'.$proxyUrl.'].');
        }
    }
    ...
    {
        if (($loopback = $this->crestLoopback()) !== CREST_LOOPBACK) {
    ...
    /**
     * Return Crest home path constant.
     */
    public function crestHomePath(): string
    {
        return CREST_HOME_PATH;
    }

    /**
     * Return Crest loopback configuration.
     */
    public function crestLoopback(): string
    {
    ...
    {
        return '/Library/LaunchDaemons/com.laravel.crest.loopback.plist';
    }
    ...
    {
        return $this->crestHomePath().'/Nginx'.($additionalPath ? '/'.$additionalPath : '');
    }

    /**
     * Get the path to the linked Crest sites.
     */
    public function sitesPath(?string $link = null): string
    {
        return $this->crestHomePath().'/Sites'.($link ? '/'.$link : '');
    }

    /**
     * Get the path to the Crest CA certificates.
     */
    public function caPath(?string $caFile = null): string
    {
        return $this->crestHomePath().'/CA'.($caFile ? '/'.$caFile : '');
    }

    /**
     * Get the path to the Crest TLS certificates.
     */
    public function certificatesPath(?string $url = null, ?string $extension = null): string
    {
        ...
        return $this->crestHomePath().'/Certificates'.$url.$extension;
    }
    ...
    /**
     * Replace Loopback configuration line in Crest site configuration file contents.
     */
    public function replaceLoopback(string $siteConf): string
    {
        ...
        $str = '#listen CREST_LOOPBACK:80; # crest loopback';

        return str_replace(
            $str,
            substr(str_replace('CREST_LOOPBACK', $loopback, $str), 1),
            $siteConf
        );
    }
    ...
    public function replaceSockFile(string $siteConf, string $phpVersion): string
    {
        ...
        $siteConf = preg_replace('/crest[0-9]*.sock/', $sockFile, $siteConf);
    ...
    /**
     * Get configuration items defined in .crestrc for a site.
     */
    public function crestRc(string $siteName, ?string $cwd = null): array
    {
        if ($cwd) {
            $path = $cwd.'/.crestrc';
        } elseif ($site = $this->parked()->merge($this->links())->where('site', $siteName)->first()) {
            $path = data_get($site, 'path').'/.crestrc';
        } else {
            return [];
        }
    ...
    /**
     * Get PHP version from .crestrc or .crestphprc for a site.
     */
    public function phpRcVersion(string $siteName, ?string $cwd = null): ?string
    {
        if ($cwd) {
            $oldPath = $cwd.'/.crestphprc';
        } elseif ($site = $this->parked()->merge($this->links())->where('site', $siteName)->first()) {
            $oldPath = data_get($site, 'path').'/.crestphprc';
        } else {
            return null;
        }
        ...
        $crestRc = $this->crestRc($siteName, $cwd);

        return PhpFpm::normalizePhpVersion(data_get($crestRc, 'php'));
    }
}
