...
abstract class CrestDriver
{
     ...
     */
    public static function assign(string $sitePath, string $siteName, string $uri): ?CrestDriver
    {
        $drivers = [];
        ...
        // Queue Crest-shipped drivers
        $drivers[] = 'Specific\StatamicCrestDriver';
        $drivers[] = 'LaravelCrestDriver';
        $drivers = array_unique(array_merge($drivers, $specificDrivers));
        $drivers[] = 'BasicWithPublicCrestDriver';
        $drivers[] = 'BasicCrestDriver';

        foreach ($drivers as $driver) {
            if ($driver === 'LocalCrestDriver') {
                $driver = new $driver;
            } else {
                $className = "Crest\Drivers\\{$driver}";
    ...
    {
        if (! file_exists($sitePath.'/LocalCrestDriver.php')) {
            return null;
        }

        require_once $sitePath.'/LocalCrestDriver.php';

        return 'LocalCrestDriver';
    }
    ...
    public static function driversIn(string $path): array
    {
        ...
        $regex = new RegexIterator($iterator, '/^.+CrestDriver\.php$/i', RecursiveRegexIterator::GET_MATCH);
    ...
    /**
     * Get all of the specific drivers shipped with Crest.
     */
    ...
    public static function customDrivers(): array
    {
        return array_map(function ($item) {
            return 'Custom\\'.$item;
        }, static::driversIn(CREST_HOME_PATH.'/Drivers'));
    }
    ...
    public function serveStaticFile(string $staticFilePath, string $sitePath, string $siteName, string $uri): void
    {
        ...
        header('X-Accel-Redirect: /'.CREST_STATIC_PREFIX.$staticFilePath);
    }
    ...
    {
        $varFilePath = $sitePath.'/.crest-env.php';
        if (! file_exists($varFilePath)) {
            $varFilePath = CREST_HOME_PATH.'/.crest-env.php';
        }
        if (! file_exists($varFilePath)) {
            return;
        }
