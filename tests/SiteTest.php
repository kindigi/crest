...
class SiteTest extends Yoast\PHPUnitPolyfills\TestCases\TestCase
{
    ...
    public function test_get_certificates_will_return_with_multi_segment_tld()
    {
        ...
        $files->shouldReceive('scandir')
            ->once()
            ->with($certPath = '/Users/testuser/.config/crest/Certificates')
            ->andReturn(['helloworld.multi.segment.tld.com.crt']);
        $files->shouldReceive('ensureDirExists')
            ->once()
            ->with('/Users/testuser/.config/crest/Certificates', user());
    ...
    {
        $files = Mockery::mock(Filesystem::class);
        $files->shouldReceive('ensureDirExists')->once()->with(CREST_HOME_PATH.'/Sites', user());
        $config = Mockery::mock(Configuration::class);
        $config->shouldReceive('prependPath')->once()->with(CREST_HOME_PATH.'/Sites');
        $files->shouldReceive('symlinkAsUser')->once()->with('target', CREST_HOME_PATH.'/Sites/link');

        swap(Filesystem::class, $files);
        swap(Configuration::class, $config);

        $linkPath = resolve(Site::class)->link('target', 'link');
        $this->assertSame(CREST_HOME_PATH.'/Sites/link', $linkPath);
    }
    ...
    public function test_add_proxy()
    {
        $config = Mockery::mock(Configuration::class);
        $config->shouldReceive('read')
            ->andReturn(['tld' => 'test', 'loopback' => CREST_LOOPBACK]);
    ...
    public function test_gets_site_url_from_directory()
    {
        ...
        $config->shouldReceive('read')
            ->andReturn(['tld' => 'test', 'loopback' => CREST_LOOPBACK, 'paths' => []]);
    ...
    public function test_can_install_nginx_site_config_for_specific_php_version()
    {
        ...
        $files->shouldReceive('get')
            ...
            ->andReturn('# '.ISOLATED_PHP_VERSION.'=php@7.4'.PHP_EOL.'server { fastcgi_pass: crest74.sock }');

        $files->shouldReceive('putAsUser')
            ->once()
            ->withArgs([
                $siteMock->nginxPath('site1.test'),
                '# '.ISOLATED_PHP_VERSION.'=php@8.0'.PHP_EOL.'server { fastcgi_pass: crest80.sock }',
            ]);
        ...
        $files->shouldReceive('getStub')
            ->once()
            ->with('site.crest.conf')
            ->andReturn(file_get_contents(__DIR__.'/../cli/stubs/site.crest.conf'));
            ...
            ->withArgs([
                ...
                Mockery::on(function ($argument) {
                    return preg_match('/^# '.ISOLATED_PHP_VERSION.'=php@8.0/', $argument)
                        && preg_match('#fastcgi_pass "unix:.*/crest80.sock#', $argument)
    ...
    public function test_replace_sock_file_in_nginx_config()
    {
        ...
        // When switching to php71, crest71.sock should be replaced with crest.sock;
        // isolation header should be prepended
        $this->assertEquals(
            '# '.ISOLATED_PHP_VERSION.'=71'.PHP_EOL.'server { fastcgi_pass: crest71.sock }',
            $site->replaceSockFile('server { fastcgi_pass: crest71.sock }', '71')
        );

        // When switching to php72, crest.sock should be replaced with crest72.sock
        $this->assertEquals(
            '# '.ISOLATED_PHP_VERSION.'=72'.PHP_EOL.'server { fastcgi_pass: crest72.sock }',
            $site->replaceSockFile('server { fastcgi_pass: crest.sock }', '72')
        );

        // When switching to php73 from php72, crest72.sock should be replaced with crest73.sock;
        // isolation header should be updated to php@7.3
        $this->assertEquals(
            '# '.ISOLATED_PHP_VERSION.'=73'.PHP_EOL.'server { fastcgi_pass: crest73.sock }',
            $site->replaceSockFile('# '.ISOLATED_PHP_VERSION.'=72'.PHP_EOL.'server { fastcgi_pass: crest72.sock }', '73')
        );

        // When switching to php72 from php74, crest72.sock should be replaced with crest74.sock;
        // isolation header should be updated to php@7.4
        $this->assertEquals(
            '# '.ISOLATED_PHP_VERSION.'=php@7.4'.PHP_EOL.'server { fastcgi_pass: crest74.sock }',
            $site->replaceSockFile('# '.ISOLATED_PHP_VERSION.'=72'.PHP_EOL.'server { fastcgi_pass: crest.sock }', 'php@7.4')
        );
    }
    ...
    public function test_it_can_read_crest_rc_files()
    {
        ...
        ], $site->crestRc('site-w-crestrc-1'));

        $this->assertEquals([
            'php' => 'php@8.1',
        ], $site->crestRc('site-w-crestrc-2'));

        $this->assertEquals([
            'item' => 'value',
            'php' => 'php@8.2',
        ], $site->crestRc('site-w-crestrc-3'));
    }
    ...
    public function test_it_can_read_php_rc_version()
    {
        ...
        $this->assertEquals('php@8.1', $site->phpRcVersion('site-w-crestphprc-1'));
        $this->assertEquals('php@8.0', $site->phpRcVersion('site-w-crestphprc-2'));
        $this->assertEquals(null, $site->phpRcVersion('my-best-site'));
        $this->assertEquals(null, $site->phpRcVersion('non-existent-site'));
        $this->assertEquals('php@8.0', $site->phpRcVersion('site-w-crestrc-1'));
        $this->assertEquals('php@8.1', $site->phpRcVersion('site-w-crestrc-2'));
        $this->assertEquals('php@8.2', $site->phpRcVersion('site-w-crestrc-3'));
        $this->assertEquals('php@8.2', $site->phpRcVersion('blabla', __DIR__.'/fixtures/Parked/Sites/site-w-crestrc-3'));
    }
}
...
class FixturesSiteFake extends Site
{
    ...
    {
        $this->crestHomePath = __DIR__.'/fixtures/'.$fixtureName;
    }
    ...
    {
        $this->crestHomePath = __DIR__.'/output';
    }
