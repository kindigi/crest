...
class NginxTest extends Yoast\PHPUnitPolyfills\TestCases\TestCase
{
    ...
    public function test_install_nginx_configuration_places_nginx_base_configuration_in_proper_location()
    {
        ...
        $files->shouldReceive('putAsUser')->andReturnUsing(function ($path, $contents) {
            ...
            $this->assertStringContainsString('include "'.CREST_HOME_PATH.'/Nginx/*"', $contents);
        })->once();
    ...
    public function test_install_nginx_directories_creates_location_for_site_specific_configuration()
    {
        ...
        $files->shouldReceive('isDir')->with(CREST_HOME_PATH.'/Nginx')->andReturn(false);
        $files->shouldReceive('mkdirAsUser')->with(CREST_HOME_PATH.'/Nginx')->once();
        $files->shouldReceive('putAsUser')->with(CREST_HOME_PATH.'/Nginx/.keep', "\n")->once();

        swap(Filesystem::class, $files);
        swap(Configuration::class, $config = Mockery::spy(Configuration::class, ['read' => ['tld' => 'test', 'loopback' => CREST_LOOPBACK]]));
    ...
    {
        $files = Mockery::mock(Filesystem::class);
        $files->shouldReceive('isDir')->with(CREST_HOME_PATH.'/Nginx')->andReturn(true);
