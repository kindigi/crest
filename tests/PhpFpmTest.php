...
class PhpFpmTest extends Yoast\PHPUnitPolyfills\TestCases\TestCase
{
    ...
    public function test_fpm_is_configured_with_the_correct_user_group_and_port()
    {
        ...
        $this->assertStringContainsString(PHP_EOL.'listen = '.CREST_HOME_PATH.'/crest72.sock', $contents);
    ...
    {
        $this->assertEquals('crest72.sock', resolve(PhpFpm::class)->fpmSockName('php@7.2'));
        $this->assertEquals('crest72.sock', resolve(PhpFpm::class)->fpmSockName('php@72'));
        $this->assertEquals('crest72.sock', resolve(PhpFpm::class)->fpmSockName('php72'));
        $this->assertEquals('crest72.sock', resolve(PhpFpm::class)->fpmSockName('72'));
    }
    ...
    public function test_utilized_php_versions()
    {
        ...
        $sites = [
            [
                ...
                'conf' => '# '.ISOLATED_PHP_VERSION.'=71'.PHP_EOL.'crest71.sock',
            ],
            [
                'site' => 'isolated-site-72.test',
                'conf' => '# '.ISOLATED_PHP_VERSION.'=php@7.2'.PHP_EOL.'crest72.sock',
            ],
            [
                'site' => 'isolated-site-73.test',
                'conf' => '# '.ISOLATED_PHP_VERSION.'=73'.PHP_EOL.'crest.sock',
            ],
        ];

        foreach ($sites as $site) {
            $fileSystemMock->shouldReceive('get')->once()->with(CREST_HOME_PATH.'/Nginx/'.$site['site'])->andReturn($site['conf']);
        }
    ...
    public function test_use_version_will_convert_passed_php_version()
    {
        ...
        $filesystem->shouldReceive('unlink')->with(CREST_HOME_PATH.'/crest.sock');

        $cli->shouldReceive('quietly')->with('sudo rm '.CREST_HOME_PATH.'/crest.sock');
    ...
    public function test_isolate_will_throw_if_site_is_not_parked_or_linked()
    {
        ...
        $this->expectException(DomainException::class);
        $this->expectExceptionMessage("The [test] site could not be found in Crest's site list.");
